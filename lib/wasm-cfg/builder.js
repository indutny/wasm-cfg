...
var MEMORY_LOAD_FLAG = 1;
var MEMORY_STORE_FLAG = 2;
...
CFGBuilder.prototype.buildFunction = function buildFunction(node) {
  ...
  this.cfg = cfg;
  this.fn = node;
  this.ssa = { memory: this.fn.localCount };
  this.exits = [];
  this.visited = 0;

  var start = this.cfg.block('start');

  var memory = this.cfg.add('memory');
  var memStore = this.setMemory(memory);

  this.buildParamSlots(node);
  this.buildBlock(node);
  if (this.cfg.currentBlock.getLastControl() === this.cfg.currentBlock)
    this.exits.push(this.cfg.currentBlock);

  this.cfg.setCurrentBlock(this.exits[0]);
  for (var i = 1; i < this.exits.length; i++) {
    var exit = this.exits[i];
    // Skip dangling nodes
    if (exit.nodes.length === 0 && exit.predecessors.length === 0)
      continue;
    this.merge(this.cfg.currentBlock, exit);
  }

  if (!(this.visited & (MEMORY_LOAD_FLAG | MEMORY_STORE_FLAG))) {
    start.remove(0);
    start.remove(0);
    this.cfg.remove(memory);
    this.cfg.remove(memStore);
  }
...
CFGBuilder.prototype.getMemory = function getMemory() {
  return this.cfg.add('ssa:load').addLiteral(this.ssa.memory);
};

CFGBuilder.prototype.setMemory = function setMemory(value) {
  return this.cfg.add('ssa:store').addLiteral(this.ssa.memory).addInput(value);
};

CFGBuilder.prototype.reloadMemory = function reloadMemory() {
  this.visited |= MEMORY_STORE_FLAG;
  var mem = this.cfg.addControl('memory');
  return this.setMemory(mem);
};
...
CFGBuilder.prototype.buildBuiltin = function buildBuiltin(expr, type) {
  ...
  if (signature.memory) {
    args.unshift(this.getMemory());
    this.visited |= MEMORY_LOAD_FLAG;
  }

  var out;
  if (signature.memory === 'store') {
...
CFGBuilder.prototype.buildCall = function buildCall(expr, type) {
  ...
  var res = this.cfg.addControl(sig.result + '.call', memory)
