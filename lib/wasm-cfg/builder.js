...
CFGBuilder.prototype.buildFunction = function buildFunction(node) {
  ...
  if (this.cfg.currentBlock.getLastControl() === this.cfg.currentBlock) {
    assert.equal(this.fn.result.name, 'void',
                 'Implicit return from non-`void` fn');
...
CFGBuilder.prototype.buildForever = function buildForever(stmt) {
  var startBlock = this.cfg.currentBlock;

  // Ensure no X intersections in graph
  if (startBlock.opcode === 'start' || startBlock.predecessors.length === 2)
    startBlock = this.jumpFrom(startBlock);
  ...
  this.cfg.currentBlock.jump(startBlock);
};
