...
function registerMemory(memory, result, name, a, b) {
...
types.forEach(function(type) {
  ...
  // addr.from_i64(i64)
  register('addr', 'from_' + type, type);
  // i64.from_addr(addr)
  register(type, 'from_addr', 'addr');
  register(type, 'const');
});

register('addr', 'add', 'addr', 'addr');
register('addr', 'sub', 'addr', 'addr');
...
[ 'i32', 'i64', 'f32', 'f64' ].forEach(function(type) {
  registerMemory('load', type, 'load', 'addr');
  registerMemory('store', type, 'store', 'addr', type);
  if (type === 'i32' || type === 'i64') {
    registerMemory('load', type, 'load8_s', 'addr');
    registerMemory('load', type, 'load8_u', 'addr');
    registerMemory('load', type, 'load16_s', 'addr');
    registerMemory('load', type, 'load16_u', 'addr');
    registerMemory('store', type, 'store8', 'addr', 'i8');
    registerMemory('store', type, 'store16', 'addr', 'i16');
  }
});
registerMemory('load', 'i64', 'load32_s', 'addr');
registerMemory('load', 'i64', 'load32_u', 'addr');
registerMemory('store', 'i64', 'store32', 'addr', 'i32');

registerMemory('store', 'addr', 'resize_memory', 'addr');
